  ...
  return (
    <aside className="flex flex-col gap-4 rounded-2xl border border-primary/35 bg-secondary p-4 shadow-sm sm:p-5" aria-label="Biztonságos Stripe fizetés">
      ...
      <div className="flex flex-col gap-3 border-t border-border pt-4">
        ...
        <div className="flex w-fit max-w-full items-center overflow-hidden rounded-xl border border-border bg-card px-2 shadow-sm" aria-label="Visa, Mastercard és Stripe">
          {paymentBrands.map((brand, index) => (
            <span key={brand.name} className={`flex h-14 items-center justify-center px-4 sm:px-5 ${index > 0 ? 'border-l border-border' : ''}`}>
              <img src={brand.src} alt={`${brand.name} logó`} width={96} height={36} className={`${brand.className} max-w-24 object-contain`} />
            </span>
          ))}
        </div>
