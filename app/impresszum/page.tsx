...
import { Building2, Mail, MapPin, ReceiptText, X } from 'lucide-react'
...
export default function ImprintPage() {
  return <AppShell><div className="page-wrap"><article className="relative mx-auto w-full max-w-4xl rounded-2xl border-2 border-border bg-card p-6 pt-20 shadow-sm md:p-10 md:pt-20">
    ...
    <section className="mt-8 rounded-xl bg-secondary p-6 text-secondary-foreground" aria-labelledby="contact-title"><h2 id="contact-title" className="text-xl font-black text-primary">Kapcsolat</h2><div className="mt-4 flex flex-col gap-3"><a className="flex min-h-11 items-center gap-3 font-bold underline-offset-4 hover:underline" href={`mailto:${operator.email}`}><Mail className="size-5" aria-hidden="true" />{operator.email}</a></div></section>

    <section className="mt-8 flex flex-col gap-3 text-base leading-relaxed md:text-lg" aria-labelledby="service-title"><h2 id="service-title" className="text-2xl font-black text-primary">A szolgáltatás adatai</h2><p>Szolgáltatás neve és webhely: Mesterek · mesterek.eu</p><p>Tevékenység: online szakemberkereső és szakember-adatlap szolgáltatás.</p><p><strong>Webtárhely-szolgáltató:</strong> Vercel Inc., 440 N Barranca Ave #4133, Covina, CA 91723, USA.</p><p>Számlázás és fizetés: Stripe Billing. A könyvelési folyamat a Bokio használatával, a svéd egyéni vállalkozásokra (enskild firma) vonatkozó szabályok szerint történik.</p><p>Adózási keret: EU-n belüli B2B szolgáltatás; érvényes magyar közösségi adószám esetén fordított adózás (reverse charge / omvänd skattskyldighet) alkalmazható.</p></section>
